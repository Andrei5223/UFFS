...
#crio uma classe para descrever um produto
class produto:
    codigo = None
    nome = None
    tipo = None
...
#crio uma classe para armazenar os dados de compras efetuadas
class compra:
    login = None
    data = None
    valorTotal = None
...
#crio as funções disponíveis no menu principal
def cadastro():             #cadastrar um novo produto
    ...
    while novoProduto.disponivel != 0 and novoProduto.disponivel != 1:                           #verifico se o valor inserido é válido
        novoProduto.disponivel = verificaInt(input("\nDigite 1 para Sim.\nDigite 0 para Não.\n"))
...
def consulta(codigo):       #consultar um produto já cadastrado, recebe o código do produto
    ...
    for i in range(len(produtos)):
        if codigo == produtos[i].codigo:            #verifico se o código está cadastrado
            print("\nProduto encontrado:")
            print(f"Título: {produtos[i].nome}")         #imprimo o nome
            print(f"Categoria: {produtos[i].tipo}")      #imprimo o tipo
            print(f"Preço: R${produtos[i].preco}")       #imprimo o preço
            print(f"Produto {produtos[i].disponivel}")   #imprimo a disponibilidade
            verificacao = 1
    if verificacao == 0:                                #se não estiver cadastrado o loop não vai imprimir nada, se ele n imprimir a variavel "verificacao" não vai ser alterada
        print("Produto não cadastrado.")
        global nao_cadastrado
        nao_cadastrado = True

def atualizar():            #atualizar o cadastro de um produto.
    global produtos
    global nao_cadastrado
    ...
    if nao_cadastrado:
        ...
        while atualiza > 0 and atualiza < 5:        #verifico se o valor inserido faz parte de uma ação e executo ela
            if atualiza == 1:
                for i in range(len(produtos)):
            ...
            if atualiza == 2:
                for i in range(len(produtos)):
                    if codigo == produtos[i].codigo:
                        ...
                        while produtos[i].tipo < 1 or produtos[i].tipo > 3:              #verifico se o valor inserido é válido
                            produtos[i].tipo = verificaInt(input("Digite a nova categoria do produto. \n1 para série.\n2 para filme.\n3 para documentário.\n"))
            ...
            if atualiza == 4:
                for i in range(len(produtos)):
                    if codigo == produtos[i].codigo:        
                        ...
                        while produtos[i].disponivel != 0 and produtos[i].disponivel != 1:                           #verifico se o valor inserido é válido
...
def filtrarLista(dados, maiorcod, maiortit, maiorpreco, maiorcat):
...
def relatorio():            #exibe um relatorios dos produtos cadastrados
    #declaro e crio as variáveis necessárias
    global produtos
    global produtosFiltrados
    maiortit = 6            
    ...
    if opcao < 0 or opcao > 5:       #verifico se o valor inserído é válido
        return
...
def comprar():              #efetua a compra de produtos
    ...
    while codigo != -1:
        ...
        if nao_cadastrado:              #verifica se está cadastrado
            nao_cadastrado = False
        else:
...
def verificaInt(entrada):   #verifica se o parâmetro recebido pode ser int e o transforma
    while True:
        try:
            ...
            entrada = input("Valor inválido, digite um inteiro: ")
            continue
        if type(entrada) == int:
            break
    return entrada
...
while ação > 0 and ação < 7:
    if ação == 1:
        ...
        while True:
            continuar = verificaInt(input("\nDigite -1 para retornar ao menu ou um código para consulta: "))
            if continuar == -1:
                break
            else:
                consulta(continuar)
