...
select sf.names, sd.names, r.dist from route r join station sf on r.idsfrom = sf.ids join station sd on r.idsto = sd.ids;
...
insert into train (idtr,nametr,capac,typet) values (6,'Arrow',95,2);

3. Incluir uma nova estação:
Name: Candy Jar

insert into station (ids,names) values (8,'Candy Jar');

4. Incluir uma nova rota:
Edgar Codd (id = 6)-> Candy Jar(id=8) Distance: 33

insert into route (idsfrom,idsto,dist) values (6,8,33);

5. Incluir uma nova linha para a rota recem criada:
Train: Arrow Data: 01/11/2023

insert into train_route(idtr,idsfrom,idsto,datet) values (6,6,8,'01/11/2023');
...
select sf.names, sd.names, r.dist, tr.nametr, ty.descr, trr.datet from 
route r join station sf on r.idsfrom = sf.ids 
	join station sd on r.idsto = sd.ids
	join train_route trr on r.idsfrom=trr.idsfrom and r.idsto = trr.idsto
	join train tr on trr.idtr = tr.idtr
	join type ty on tr.typet = ty.idt;
//mesma coisa:
select sf.names, st.names, r.dist, t.nametr, ty.descr, tr.datet
from train_route tr join train t on tr.idtr=t.idtr
		    join type ty on t.typet=ty.idt
		    join route r on tr.idsfrom=r.idsfrom and tr.idsto=r.idsto
		    join station sf on r.idsfrom=sf.ids
